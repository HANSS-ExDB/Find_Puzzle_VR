...
namespace BlockPuzzleGameTemplate
{
    public class BlockTile : MonoBehaviour
    {
        ...
        private void Start ()
        {
            ...
            FeatureManager_ = FindObjectOfType<FeatureManager>();
        }

        private void OnMouseDown ()
        {
            if (IsPointerOverUIObjectPos(Input.mousePosition))
            {
                // Exit if the pointer is over a UI element
                return;
            }
        ...
        void Clicked ()
        {
            if (!FeatureManager_.CanRotateClockWise())
            {
                Debug.LogWarning("Feature is not yet unlocked!");
                return;
            }
        ...
        void DoubleClicked ()
        {
            if (!FeatureManager_.CanRotateCounterClockWise())
            {
                Debug.LogWarning("Feature is not yet unlocked!");
                return;
            }
        ...
        public bool IsPointerOverUIObjectPos ( Vector2 ThePos )
        {
            PointerEventData eventDataCurrentPosition = new PointerEventData(EventSystem.current);
            eventDataCurrentPosition.position = ThePos;
